...
abstract class BaseUrlAuth implements iUrlAuth
{
    ...
    public function sign($url, $expiration, $timeunit)
    {
        ...
        $expiration = $this->getExpirationTimestamp($expiration, $timeunit);
     ...
     *        - int: The expiration time will be set to X days, X hours, X mintues, X seconds from now
     *
     * @param string $timeunit (days, hours, mintues, seconds)
     * @throws \abovesky\UrlAuth\Exceptions\InvalidExpiration
     *
     * @return string
     */
    protected function getExpirationTimestamp($expiration, $timeunit = 'days')
    {
        if (is_int($expiration)) {
            $expiration = (new DateTime())->modify((int) $expiration.' '.$timeunit);
        }
